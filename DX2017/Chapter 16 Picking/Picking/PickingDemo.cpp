...
void PickingApp::BuildMeshGeometryBuffers()
{
	...
	XMStoreFloat3(&mSphere.Center, 0.5f*(vMin + vMax));
	mSphere.Radius = XMVectorGetX(XMVector3Length(0.5*(vMax - vMin)));
...
void PickingApp::Pick(int sx, int sy)
{
	...
	mPickedTriangle = -1;
	float tmin = 0.0f;
	if(XNA::IntersectRaySphere(rayOrigin, rayDir, &mSphere, &tmin))
	{
