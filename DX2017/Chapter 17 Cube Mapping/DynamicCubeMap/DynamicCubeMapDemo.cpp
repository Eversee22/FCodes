  ...
  mSkullIndexCount(0), mLightCount(3)
{
	...
	mCenterSphereMat.Specular = XMFLOAT4(0.6f, 0.6f, 0.6f, 16.0f);
	mCenterSphereMat.Reflect  = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
}
...
bool DynamicCubeMapApp::Init()
{
	...
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice,
		L"Textures/floor.dds", &texRes, &mFloorTexSRV, 0));
	ReleaseCOM(texRes);

	HR(DirectX::CreateDDSTextureFromFile(md3dDevice,
		L"Textures/stone.dds", &texRes, &mStoneTexSRV, 0));
	ReleaseCOM(texRes);

	HR(DirectX::CreateDDSTextureFromFile(md3dDevice,
		L"Textures/bricks.dds", &texRes, &mBrickTexSRV, 0));
	ReleaseCOM(texRes);
