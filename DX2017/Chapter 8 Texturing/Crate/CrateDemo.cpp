...
: D3DApp(hInstance), mBoxVB(0), mBoxIB(0), mDiffuseMapSRV(0), mDiffuseMapSRV2(0), mEyePosW(0.0f, 0.0f, 0.0f),
  mTheta(1.3f*MathHelper::Pi), mPhi(0.4f*MathHelper::Pi), mRadius(2.5f), mRotAng(0.0f)
{
...
bool CrateApp::Init()
{
	...
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice,
		L"../flare.dds", &texResource, &mDiffuseMapSRV));
	ReleaseCOM(texResource);

	HR(DirectX::CreateDDSTextureFromFile(md3dDevice,
		L"../flarealpha.dds", nullptr, &mDiffuseMapSRV2));
...
void CrateApp::UpdateScene(float dt)
{
	...
	//mRotAng += 20.0f*dt;
	//XMStoreFloat4x4(&mTexTransform,
	//	XMMatrixTranslation(-0.5f, -0.5f, 0.0f)*
	//	XMMatrixRotationZ(mRotAng)*
	//	XMMatrixTranslation(0.5f, 0.5f, 0.0f));
}
