...
bool SkinnedMeshApp::Init()
{
	...
	mSky  = new Sky(md3dDevice, texRootDir + L"desertcube1024.dds", 5000.0f);
	mSmap = new ShadowMap(md3dDevice, SMapSize, SMapSize);

	mCam.SetLens(0.25f*MathHelper::Pi, AspectRatio(), 1.0f, 1000.0f);
	mSsao = new Ssao(md3dDevice, md3dImmediateContext, mClientWidth, mClientHeight, mCam.GetFovY(), mCam.GetFarZ());

	mStoneTexSRV = mTexMgr.CreateTexture(texRootDir + L"floor.dds");
	mBrickTexSRV = mTexMgr.CreateTexture(texRootDir + L"bricks.dds");
	mStoneNormalTexSRV = mTexMgr.CreateTexture(texRootDir + L"floor_nmap.dds");
	mBrickNormalTexSRV = mTexMgr.CreateTexture(texRootDir + L"bricks_nmap.dds");

	BuildShapeGeometryBuffers();
	BuildSkullGeometryBuffers();
	BuildScreenQuadGeometryBuffers();

	mCharacterModel = new SkinnedModel(md3dDevice, mTexMgr, "Models\\soldier.m3d", texRootDir);
